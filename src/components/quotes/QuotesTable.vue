...
async function loadQuotes() {
    ...
    quotes.value.forEach((quote) => listItems.value.push({ quote: quote }));

    await characterServices.findAll().then((characters) => {
        characters.forEach((c) => {
            const item = listItems.value.find(
                (i) => i.quote.characterID == c.id
            );
...
async function remove(item) {
    ...
    await quoteServices.delete(item.quote.id).then(() => {
        ...
        let quoteIndex = quotes.value.findIndex(
            (h) => h.id == item.quote.id
        );
...
async function save(quoteData, characterData) {
    isInputDisabled.value = true;
    ...
    await quoteServices.update(quoteData).then((response) => {
        ...
        let itemIndex = listItems.value.findIndex((i) => i.quote.id == quoteData.id);
        const listItem = { quote: quoteData, characterName: characterData.name };
